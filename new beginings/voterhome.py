        ...
        try:
            ...
            for candidate in candidates:
                ...
                if pid not in self.voted_candidates:  # Check if the user has not voted for this candidate
                    vote_button =ctk.CTkButton(profile_frame, text="Vote", command=lambda p=pid: self.vote_candidate(p), bg_color="black",  width=100)
                    vote_button.pack(side=tk.RIGHT)
                else:
                    vote_button = ctk.CTkButton(profile_frame, text="Voted", state=tk.DISABLED, bg_color="black",  width=100)
                    vote_button.pack(side=tk.RIGHT)
...
if len(sys.argv) >= 3:
    ...
    print("PID and Password are missing.")
    sys.exit()
